...
static int ntfs_long_to_errno(long status)
{
	ntfs_log_trace("Converting w32error 0x%x.\n",status);
	switch (status) {
 ...
 */
static __inline int ntfs_device_unix_status_flags_to_win32(int flags)
{
	int win_mode;

	switch (flags & GENERIC_ALL) {
	case O_RDONLY:
 ...
 *	 -1 if not, and errno set.
 */
static __inline int ntfs_device_win32_open_file(char *filename, win32_fd *fd,
		int flags)
{
	HANDLE handle;
	int mode;
 ...
 *        -1 if not, and errno set.
 */
static __inline int ntfs_device_win32_open_drive(int drive_id, win32_fd *fd,
		int flags)
{
	HANDLE handle;
	int err;
 ...
 *        -1 if not, and errno set.
 */
static __inline int ntfs_device_win32_open_lowlevel(int drive_id,
		...
		const s64 count, void *rbuf, const void *wbuf)
{
	...
	ntfs_log_trace("pos = 0x%llx, count = 0x%llx, direction = no idea.\n",
	...
	if (fd->ntdll) {
		...
		if (!res) {
			errno = ntfs_w32error_to_errno(GetLastError());
			ntfs_log_trace("SOmethingFile() failed.\n");
			return -1;
		}
		if (rbuf && !pos) {
 ...
 */
static __inline int ntfs_win32_hdio_getgeo(struct ntfs_device *dev,
 ...
 *	 -1 if not, and errno set.  Note if error fd->handle is trashed.
 */
static __inline int ntfs_win32_blksszget(struct ntfs_device *dev,int *argp)
{
	win32_fd *fd = (win32_fd *)dev->d_private;
	DWORD bytesReturned;
	DISK_GEOMETRY dg;
