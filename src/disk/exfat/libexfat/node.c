...
static bool check_entries(const struct exfat_entry* entry, int n)
{
	...
	for (i = 0; i < n + 1; previous = current, i++)
	{
		...
		case EXFAT_ENTRY_FILE_NAME:
			...
			break;
		default:
			if (EXFAT_ENTRY_FILE_TAIL <= previous && previous <= 0xff) {
...
{
	struct exfat_entry* entries = malloc(sizeof(struct exfat_entry) * n);
	if (entries == NULL) {
		return -ENOMEM;
	}
	int rc;
...
{
	struct exfat_entry* entries = malloc(sizeof(struct exfat_entry) * (1 + node->continuations));
	if (entries == NULL) {
		return -ENOMEM;
	}
	...
	if (node->parent == NULL) {
		rc = 0; /* do not flush unlinked node */
		goto err;
	}
		...
		const le16_t* name, off_t offset, uint16_t attrib)
{
	...
	struct exfat_entry* entries = malloc(sizeof(struct exfat_entry) * (2 + name_entries));
	if (entries == NULL) {
		return -ENOMEM;
	}
	struct exfat_entry_meta1* meta1 = (struct exfat_entry_meta1*) &entries[0];
