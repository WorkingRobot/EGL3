...
/**
 * ntfs_attr_nonresident_data_reserve - add or update the $DATA attribute as non-resident and reserve space
 * @ni:		opened ntfs inode to which add attribute
 * @size:	size of the new $DATA attribute
 *
 ...
 */
int ntfs_attr_nonresident_data_reserve(ntfs_attr* na, s64 size)
{
	int err;
	if (!na) {
		err = errno;
		ntfs_log_perror("Failed to open attribute");
		goto err_out;
	}
	/* Resize and set attribute value. */
	if (ntfs_non_resident_attr_expand(na, size, HOLES_NO)) {
		err = errno;
		ntfs_log_perror("Failed to initialize attribute");
		ntfs_attr_close(na);
		goto err_out;
	}
	return 0;
...
/**
 * ntfs_attr_nonresident_data_create_reserve - add or update the $DATA attribute as non-resident and reserve space
 * @ni:		opened ntfs inode to which add attribute
 * @size:	size of the new $DATA attribute
 *
 * If inode haven't got enough space to add attribute, add attribute to one of
 ...
 * On success return 0. On error return -1 with errno set to the error code.
 */
int ntfs_attr_nonresident_data_create_reserve(ntfs_inode* ni, s64 size, ntfs_attr** o_na)
{
	const ATTR_TYPES type = AT_DATA;
	...
	if (ntfs_attr_can_be_non_resident(ni->vol, type, name, name_len)) {
		...
		ntfs_log_perror("$DATA must be non-resident");
		return -1;
	}

	attr_rec_size = offsetof(ATTR_RECORD, compressed_size) + // would be non_resident_end
	...
	if (!NInoAttrList(ni)) {
		...
		return ntfs_attr_nonresident_data_reserve(ni, size);
	}
	/* Allocate new extent. */
...
add_non_resident:
	...
	*o_na = ntfs_attr_open(ni, type, name, name_len);
	if (!*o_na) {
		err = errno;
	...
	/* Resize and set attribute value. */
	if (ntfs_non_resident_attr_expand(*o_na, size, HOLES_NO)) {
		err = errno;
		ntfs_log_perror("Failed to initialize attribute");
		ntfs_attr_close(*o_na);
		goto err_out;
	}
	return 0;
			...
			hole_type holes)
{
	...
	if (na->type==AT_STANDARD_INFORMATION || na->type==AT_ATTRIBUTE_LIST) {
		...
		if (ntfs_inode_free_space(na->ni, offsetof(ATTR_RECORD,
			compressed_size) + 8)) { // would be non_resident_end
