...
static __inline BOOL NInoFileNameTestAndClearDirtyImpl(ntfs_inode* ni) {
    const BOOL old_state = (((ni)->state) & (1 << (NI_FileNameDirty)));
    ((ni)->state) &= ~(1 << (NI_FileNameDirty));
    return old_state;
}

static __inline BOOL NInoAttrListTestAndClearDirtyImpl(ntfs_inode* ni) {
    const BOOL old_state = (((ni)->state) & (1 << (NI_AttrListDirty)));
    ((ni)->state) &= ~(1 << (NI_AttrListDirty));
    return old_state;
}

static __inline BOOL NInoTestAndClearDirtyImpl(ntfs_inode* ni) {
    const BOOL old_state = (((ni)->state) & (1 << (NI_Dirty)));
    ((ni)->state) &= ~(1 << (NI_Dirty));
    return old_state;
}
...
static int ntfs_inode_sync_in_dir(ntfs_inode *ni, ntfs_inode *dir_ni)
{
    ...
    if ((ni->mrec->flags & MFT_RECORD_IN_USE) && ni->nr_extents != -1 &&
            NInoFileNameTestAndClearDirtyImpl(ni) &&
    ...
    if ((ni->mrec->flags & MFT_RECORD_IN_USE) && ni->nr_extents != -1 &&
            NInoAttrList(ni) && NInoAttrListTestAndClearDirtyImpl(ni)) {
        ntfs_attr *na;
...
sync_inode:
    ...
    if (NInoTestAndClearDirtyImpl(ni)) {
    ...
    if (ni->nr_extents > 0) {
        ...
        for (i = 0; i < ni->nr_extents; ++i) {
            ...
            eni = ni->extent_nis[i];
            if (!NInoTestAndClearDirtyImpl(eni))
                continue;
