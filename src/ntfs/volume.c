...
ntfs_volume *ntfs_device_mount(struct ntfs_device *dev, ntfs_mount_flags flags)
{
	...
	if (need_fallback_ro) {
		NVolSetReadOnly(vol);
		ntfs_log_error("fallback_readonly_msg");
	}
...
void ntfs_mount_error(const char *volume, const char *mntpoint, int err)
{
	switch (err) {
		case NTFS_VOLUME_NOT_NTFS:
			ntfs_log_error("invalid_ntfs_msg (%s)", volume);
			break;
		case NTFS_VOLUME_CORRUPT:
			ntfs_log_error("corrupt_volume_msg");
			break;
		case NTFS_VOLUME_HIBERNATED:
			ntfs_log_error("hibernated_volume_msg (%s, %s)", volume, mntpoint);
			break;
		case NTFS_VOLUME_UNCLEAN_UNMOUNT:
			ntfs_log_error("unclean_journal_msg");
			break;
		case NTFS_VOLUME_LOCKED:
			ntfs_log_error("opened_volume_msg");
			break;
		case NTFS_VOLUME_RAID:
			ntfs_log_error("fakeraid_msg");
			break;
		case NTFS_VOLUME_NO_PRIVILEGE:
			ntfs_log_error("access_denied_msg (%s)", volume);
			break;
	}
}
