...
namespace EGL3::Utils {
    namespace {
        ...
        template<bool Insensitive = false, class Str, class Traits = std::conditional<Insensitive, ci_char_traits, std::char_traits<char>>::type>
        constexpr static std::strong_ordering CompareStrings(const Str& A, const Str& B) {
            return TraitsCompare<Traits>(A.data(), A.size(), B.data(), B.size()) <=> 0;
        }
    }
